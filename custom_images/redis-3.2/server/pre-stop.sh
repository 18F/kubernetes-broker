...
primary_server_ip_from_sentinel=$(redis-cli --raw -h $SENTINEL_HOST -p $SENTINEL_PORT SENTINEL get-master-addr-by-name primaryserver | head -1)
...
then
  ...
  redis-cli -h $SENTINEL_HOST -p $SENTINEL_PORT -c "SENTINEL failover primaryserver"
fi
